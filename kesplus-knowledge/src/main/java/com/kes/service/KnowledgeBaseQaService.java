...
public class KnowledgeBaseQaService extends ServiceImpl<KnowledgeBaseQaMapper, KnowledgeBaseQa> {
    ...
    private dev.langchain4j.model.chat.StreamingChatLanguageModel streamingModel;
    ...
    public QaResponse qa(String kbUuid, String question, Integer maxResults, Double minScore, Double temperature) {
        ...
        int promptTokens = tokenizer != null ? tokenizer.estimateTokenCount(context + question) : 0;
        int answerTokens = tokenizer != null ? tokenizer.estimateTokenCount(answer) : 0;
    ...
    public void streamQa(String kbUuid, String question, Integer maxResults, Double minScore, 
        ...
        try {
            ...
            if (streamingModel != null) {
                ...
                streamingModel.generate(prompt, tokenConsumer);
            }
            ...
            int promptTokens = tokenizer != null ? tokenizer.estimateTokenCount(context + question) : 0;
            int answerTokens = tokenizer != null ? tokenizer.estimateTokenCount(fullAnswer.toString()) : 0;
    ...
    public List<QaResponse> getHistory(String kbUuid, int limit) {
        List<KnowledgeBaseQa> records = baseMapper.selectByKbUuid(kbUuid, limit);
        List<QaResponse> responses = new ArrayList<>();
        for (KnowledgeBaseQa record : records) {
            responses.add(QaResponse.fromEntity(record));
        }
        return responses;
    }
    ...
    private String generateAnswer(String question, String context) {
        ...
        Prompt prompt = template.apply("context", context, "question", question);
    ...
    private String buildPrompt(String question, String context) {
        PromptTemplate template = PromptTemplate.from(RAG_PROMPT_TEMPLATE);
        return template.apply("context", context, "question", question).text();
    }
}